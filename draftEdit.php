...
<body style="font-family:Calibri; color:rgb(240,240,240);padding:0% ;margin:0%; width:98%;background-color:rgb(40,1,22); background-image:url('newflip1.jpg');background-repeat:no-repeat;background-size:cover; background-position:center; display:block;height:100%;">
	...
	<!-- this is the side tabs bar for navigation -->
		<div class="col-10 col-m-2"  style="height:60%;display:block;">
			...
			<div id="draft_div" class="col-1 col-m-1 sideMenu" style="background-color:rgb(255, 0, 84, 0.8);>
		...
		<div class="col-4 col-m-4" style="height:82vh;margin-left:3%;display:block; ">
			<form action="sendDraft.php" method="post" style="height:100%">
			<?php
				...
				//to return the selected mail item from the drafts database
				$sql= "SELECT * FROM ".$user."_draft WHERE id=".$id;
				$result = mysqli_query($conn, $sql);
				$row = mysqli_fetch_assoc($result);
				echo "<input type='hidden' name='id' value=".$id." />";
				echo '<input style="padding: 12px 10px;border: 1px solid #ccc;border-radius: 4px;;background-color:rgba(219, 32, 76,1); margin:0.2%; border:none;color:white; width:98%; height:20px" type="email;" name="To" placeholder="Recipients:" value="'.$row['To_email'].'"></br>';
				echo '<input style="padding: 12px 10px;border: 1px solid #ccc;border-radius: 4px;color:white;background-color:rgba(219, 32, 76,1); border:none; margin:0.2%; width:98%; height:20px" type="text" name="Subject" placeholder="Subject:" value="'.$row['Subject'].'"></br>';
				echo'<textarea name="message" style="padding: 12px 10px;border: 1px solid #ccc;border-radius: 4px;overflow-y: auto;overflow-x:hidden;color:white;background-color:rgba(219, 32, 76,1);resize: none;width:97.5%; height:70%;margin:0.2%; ">'.$row['Message'].'</textarea>';
				if($row["Attachment"]!=""){
					$files = scandir("uploads");
					for($a=2; $a<count($files);$a++){
						if($files[$a]==$row["Attachment"]){						
							echo "<a style='font-size:20px' download=".$files[$a]." href='uploads/".$files[$a]."'>".$files[$a]."</a>";
							echo "<input type='hidden' name='Attachment' value=".$files[$a]." />";					
							
						}
					}
				}else{
					echo "<input type='hidden' name='Attachment' value='' />";	
				    //echo '<input style="padding:0.6%;border-radius: 4px;border:0px solid black;float:right; margin:1%; margin-right:2%;background-color:rgb(255, 0, 84);color: white;" type="file" name="file2" value="+" style="width:80px;height: 20px;">';
					}
				?>
				</br>
				...
				<input  formaction="draftUpdate.php" style="padding:0.6%;border-radius: 4px;border:0px solid black;float:right; margin:1%; margin-right:2%;background-color:rgb(255, 0, 84);color: white;" type="submit" value="Save As Draft" style="width:80px;height: 20px;">		 
