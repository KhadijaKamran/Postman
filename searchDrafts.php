...
<?php
//startiing the session and getting the variables of current logged in user.
...
<body style="font-family:Calibri; color:rgb(240,240,240);padding:0% ;margin:0%; width:98%;background-color:rgb(40,1,22); background-image:url('newflip1.jpg');background-repeat:no-repeat;background-size:cover; background-position:center; display:block;height:100%;">
		...
		<div class="col-1 col-m-1" style="color:white; padding-top:1%;display:block; height:100%;">
			...
			<div class="col-11 col-m-5" style="height:20%;padding-top:1px;">
				<!-- For for the search bar -->

				<form action="SearchDrafts.php" onsubmit="handle">
					<input type="text" name="Search" placeholder="Search" style="width:80%; height:25px">
				</form>
				<!-- detecting the enter pressed after filling in the search bar -->
...
//getting the string sent from the search bar

				$val=$_REQUEST['Search'];
//query for searching in the DRAFTS. The search is done within the Message subject and email names.
				$sql= "SELECT * FROM `".$user."_draft` WHERE To_email like '%".$val."%' OR Subject like '%".$val."%' OR Message like '%". $val."%' ORDER BY id DESC";
				$result = mysqli_query($conn, $sql);
				if(mysqli_num_rows($result)>0){
						while($row = mysqli_fetch_assoc($result)){
							echo"<div class='col-1 col-m-1 mailItem'>
								<a href='draftEdit.php?id=".$row["id"]."' class='sideMenuLink'>
